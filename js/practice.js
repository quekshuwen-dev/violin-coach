// Practice tab: camera recording with live pitch sampling, video upload,
// Gemini feedback, and the feedback card.
import {S,setDayStars,todayIdx,esc,$} from './state.js';
...
async function stopRec(){
  ...
  let cons=0;
  if(cnt>3){
    // Consistency: spread of pitch in cents around the median (tighter = steadier)
  ...
  await callGemini(captFrame,cnt,cons,recSec,'camera recording');
}
...
function handleUpload(e){
  ...
  $('vid-preview-wrap').classList.remove('hidden');
}
function resetUpload(){
  upFile=null;$('file-in').value='';$('upload-vid').src='';
  $('upload-area').classList.remove('hidden');
  $('vid-preview-wrap').classList.add('hidden');
}
async function analyzeUpload(){
  if(!upFile)return;
  const frame=await extractFrame(upFile);
  await callGemini(frame,null,null,null,'uploaded video');
}
...
// ─── Gemini ─────────────────────────────────────────────
async function callGemini(frame,pitchCnt,cons,dur,src){
  if(!S.gemKey){
  ...
  if(!navigator.onLine){alert('AI feedback needs an internet connection. The tuner and scales still work offline!');return;}
  const overlay=$('analyzing');overlay.classList.add('show');
  const prof=S.profiles[S.cur];
  const audioLine=pitchCnt===null
    ?'Audio: not analysed for uploaded videos.'
    :`Audio: ${pitchCnt} pitch samples over ${Math.round(dur)}s, pitch steadiness ${cons.toFixed(0)}%, ${pitchCnt<5?'very little violin sound':pitchCnt<30?'some violin playing':'good amount of violin playing'}.`;
  const parts=[];
...
{"stars":4,"posture":"...","violin_hold":"...","bow_hold":"...","intonation":"...","overall":"one warm sentence","positives":["...","..."],"improvements":["...","..."],"encouragement":"fun motivating phrase"}`});
  ...
  try{
    ...
    showFeedback(fb);
    const di=todayIdx();if(di>=0){setDayStars(S.cur,di,fb.stars||0);renderWeek();}
  }catch(e){
    console.error(e);
    ...
    showFeedback(fb);
    const di=todayIdx();if(di>=0){setDayStars(S.cur,di,3.5);renderWeek();}
...
function showFeedback(fb){
  ...
  $('fb-stars').innerHTML=[1,2,3,4,5].map(i=>{
    const lit=i<=full||(half&&i===full+1);
    return`<span class="star-ic ${lit?'star-full':'star-empty'}">${lit?'★':'☆'}</span>`;
  }).join('');
  $('fb-score').textContent=`${stars} / 5 stars`;
  $('fb-trophy').textContent=stars>=4.5?'🏆':stars>=3.5?'🌟':stars>=2.5?'⭐':'💪';
  $('fb-overall').textContent=`"${fb.overall||''}"`;
  $('fb-posture').textContent=fb.posture||'';
  $('fb-hold').textContent=fb.violin_hold||'';
  $('fb-bow').textContent=fb.bow_hold||'';
  $('fb-intone').textContent=fb.intonation||'';
